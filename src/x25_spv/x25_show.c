/*
 * $Id: x25_show.c,v 1.1.1.1 1998/11/18 15:03:13 paul Exp $
 *
 * Project : PX25 - Router per Autorizzazioni POS
 *
 * Module: x25_spv
 *
 * Contents: show functions for spv
 *
 ...
 *
 * Revision 1.1  1995/09/28  15:44:41  giorgio
 * Added showing of datascope files
 *
...
static char rcsid[] = "$Id: x25_show.c,v 1.1.1.1 1998/11/18 15:03:13 paul Exp $";
...
showlog()
{
    ...
    if (choice == 0)
    {
        ...
        fflush(stdout);
        gets(dscope_file);
        sprintf(command, "%s %s/%s", pager, dscope_name, dscope_file);
    }
    else
    {
        time(&clock);
        ...
        sprintf(command, "%s %s%s%s", pager, file_name, dummy, day);
    }
